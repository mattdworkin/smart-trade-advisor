...
It routes each question to the most useful sources (market data, NewsAPI.ai headlines, vector knowledge, and company relationship graph), then produces a personalized answer and an automated implementation action plan.
...
- Pulls daily market headlines from NewsAPI.ai (with NYT fallback) and generates brief summaries.
...
- `smart_trade_agent/services/news_service.py`: NewsAPI.ai + NYT ingest + summary
...
- `NEWSAPI_AI_KEY`
- optionally `NEWS_PROVIDER` (`auto`, `newsapi_ai`, or `nyt`)
- optionally `NYT_API_KEY` for fallback
...
- If `NEWSAPI_AI_KEY` is set and `NEWS_PROVIDER=auto`, NewsAPI.ai is used first.
- If NewsAPI.ai fails or no key is set, NYT API (or NYT RSS) is used as fallback.
